                ...
                if (networkEvents.lNetworkEvents & FD_ACCEPT) // 若产生accept事件（此处与位掩码相与）
                {
                    ...
                    if (total + 1 < MAX_LINK_NUM) // 若增加一个客户端仍然小于最大连接数，则接受该链接
                    {
                        ...
                        if (newSock != INVALID_SOCKET)
                        {
                            ...
                            int usr_name_length;
                            int bytesReceived = recv(cliSock, (char *)&usr_name_length, sizeof(int), 0);
                            if (bytesReceived != sizeof(int))
                            {
                                cout << "无法接收用户名长度或连接已关闭。" << endl;
                            }
                            else
                            {
                                usr_name_length = ntohl(usr_name_length); // 将网络字节序转换为主机字节序
                                char usr_name[BUFFER_SIZE] = {0};
                                bytesReceived = recv(cliSock, usr_name, usr_name_length, 0);
                                if (bytesReceived != usr_name_length)
                                {
                                    cout << "无法接收完整用户名或连接已关闭。" << endl;
                                }
                                else
                                {
                                    cout << "接收到用户名：" << usr_name << endl;
                                }
                            }
                            // char usr_name[BUFFER_SIZE];
                            // recv(cliSock, usr_name, usr_name_length, 0); // 接收用户名内容
                            cout << "用户" << usr_name << "进入了聊天室" << endl;
                            usrs[nextIndex] = (string)usr_name;
