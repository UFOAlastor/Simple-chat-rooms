                ...
                else if (networkEvents.lNetworkEvents & FD_READ) // 接收到消息
                {
                    ...
                    for (int j = 1; j <= total; j++)
                    {
                        ...
                        if (nrecv > 0)                                           // 如果接收到的字符数大于0
                        {
                            ...
                            std::strcpy(content, str + 1);            // 将去除了开头'['的部分拷贝到content中

                            // 获取[]里面的内容
                            char inside[100];                                          // 假设最大长度为100
                            std::memset(inside, 0, sizeof(inside));                    // 初始化inside为0，方便后续处理
                            std::strncpy(inside, str + 1, std::strcspn(str + 1, "]")); // 将开头'['后面到第一个']'之前的部分拷贝到inside中
