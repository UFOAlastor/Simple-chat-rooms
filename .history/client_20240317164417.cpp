...
int main()
{
    ...
    string usr_name;
    cin >> usr_name;
    usr_name = '[' + usr_name.replace('[','').replace(']','') + ']';
    send(cliSock, usr_name.c_str(), usr_name.length(), 0); // 发送用户名称给服务器
    ...
    while (1)
    {
        ...
        send(cliSock, (usr_name + sl_name + (string)buf).c_str(), usr_name.length() + sl_name.length() + sizeof(buf), 0);
    }
    closesocket(cliSock);
    WSACleanup();
    return 0;
