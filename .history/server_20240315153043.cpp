                ...
                if (networkEvents.lNetworkEvents & FD_ACCEPT) // 若产生accept事件（此处与位掩码相与）
                {
                    ...
                    if (total + 1 < MAX_LINK_NUM) // 若增加一个客户端仍然小于最大连接数，则接受该链接
                    {
                        ...
                        if (newSock != INVALID_SOCKET)
                        {
                            ...
                            // 接收用户名称信息
                            string usr_name;
                            int nrecv = recv(cliSock[nextIndex], usr_name, sizeof(usr_name), 0);
                            usrs[nextIndex] = usr_name;
                            cout << "#" << nextIndex << "用户" << usr_name << "进入了聊天室，当前连接数：" << total << endl;

                            // 给所有客户端发送欢迎消息
                            char buf[BUFFER_SIZE] = "[聊天室]欢迎用户";
                            strcat(buf, inet_ntoa(usr_name));
                            strcat(buf, "进入聊天室");
